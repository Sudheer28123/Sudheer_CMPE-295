...
#define adcTask_FREQUENCY_MS					( 100 / portTICK_RATE_MS )



/*
 * The task used for ADC data acquisition.
 */
static void adcTask(void *adcParameters);
/*-----------------------------------------------------------*/
...
int main(void)
{
	...
	/* Creating the adcTask, which initializes the ADC module and then reads ADC data regularly. */
	xTaskCreate( adcTask, ( signed char * ) "ADC", configMINIMAL_STACK_SIZE, NULL, configADC_TASK_PRIORITY, NULL );
...
static void adcTask( void *adcParameters )
{
	...
	for(;;)
	{
        ...
        vTaskDelayUntil( &xNextWakeTime, adcTask_FREQUENCY_MS );
