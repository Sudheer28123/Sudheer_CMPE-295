...
#define	configADC_TASK_PRIORITY	( tskIDLE_PRIORITY + 1 )
#define	configULT_TASK_PRIORITY	( tskIDLE_PRIORITY + 1 )
#define configMBUS_TASK_PRIORITY (tskIDLE_PRIORITY + 1)
...
static int byt_num = 0;

uint8_t final_data[16] = {0x55,0x55,0x55,0x55,0x55,0x55,0x00,0x64,0x05,0xDC,0x5F,0x5F,0x5F,0x5F,0x5F,0x5F};

#define MBUS_FREQUENCY_MS					( 33 / portTICK_RATE_MS )
...
void MODBUS_Task( void *ModbusParam)
{
	 ...
	 eMBErrorCode    eStatus;
	    ...
	    vTaskDelayUntil( &xNextWakeTime, MBUS_FREQUENCY_MS );
	    for( ;; )
	    {
	        (void)eMBPoll();
...
int main(void)
{
    ...
    SystemInit();
		...
		xTaskCreate( MAGSEN_Task, ( signed char * ) "MA", configMINIMAL_STACK_SIZE, NULL, configADC_TASK_PRIORITY, NULL );
		xTaskCreate( ULTSEN_Task, ( signed char * ) "UL", configMINIMAL_STACK_SIZE, NULL, configULT_TASK_PRIORITY, NULL );
		xTaskCreate( MODBUS_Task, ( signed char * ) "MODBUS", configMINIMAL_STACK_SIZE, NULL, configMBUS_TASK_PRIORITY, NULL );
...
eMBErrorCode eMBRegCoilsCB( UCHAR * pucRegBuffer, USHORT usAddress, USHORT usNCoils,eMBRegisterMode eMode )
{
	...
	if(byt_num < 16)
	{
		*pucRegBuffer = final_data[byt_num];
		byt_num++;
	}
